...
④는 웹 화면 하단의 **"저장 후 수집 실행"** 버튼으로도 실행할 수 있습니다(진행 로그 실시간 표시).
...
| 처음이라면 | 더 알아보기 |
|---|---|
| [빠른 시작](#빠른-시작) → [사전 요구사항](#사전-요구사항-macos) → [웹 설정 화면](#웹-설정-화면) | [MCP로 LLM 연결](#mcp-서버--llm에서-내-볼트-쓰기-선택) · [데이터 관리·삭제](#데이터-관리--삭제) · [문제 해결](#문제-해결-troubleshooting) |
| [무엇이 만들어지나](#무엇이-만들어지나) | [에이전트로 설치](#-codexclaude-code로-설치하기) · [MCP 확장 프롬프트](#-mcp-서버chatgptclaudegemini-연동-기능-추가하기) |
...
`python3 kit.py web` → http://127.0.0.1:8765. 설정 입력부터 수집 실행까지 터미널 없이 진행할 수 있고,
자동 수집이 못 채우는 필드(관계·전화 등)는 **빈 값으로 생성**돼 여기서 채웁니다.

- **설정 탭**: 카카오 닉네임(필수 — 본인 메시지 "나" 매핑), 볼트 경로(**폴더 선택** 버튼으로
  macOS 폴더 창에서 지정 가능), **수집 소스 on/off**(카카오·SMS·메일·메모·Safari·GitHub),
...
- **채팅방 탭**: 자동 명명된 카카오 방 이름 변경·**수집 제외 체크** → 다음 `run`에서 반영.
  chat_id를 클릭하면 **최근 메시지 미리보기**로 어떤 방인지 확인할 수 있습니다
...
> **참고**: 이 중 상당 부분은 이미 구현되어 있습니다 — 로컬 stdio MCP 서버(`kit.py mcp`, 도구 30개+쓰기),
> HTTP 모드(`--http` + Bearer 토큰), 감사로그, 클라우드 이관·Cloudflare Tunnel 가이드([deploy/README.md](deploy/README.md)),
> rsync 단방향 동기화(웹 버튼). 사용법은 위 [MCP 서버 섹션](#mcp-서버--llm에서-내-볼트-쓰기-선택)을 보세요.

아래 프롬프트는 그 위에 **아직 없는 확장**을 직접 개발하고 싶을 때 Codex·Claude Code에 전달하는 개발 프롬프트입니다.

* ChatGPT 커스텀 커넥터용 `search`/`fetch` 도구·검색 인덱스
* Cloudflare Workers 기반 원격 MCP (Mac이 꺼져 있어도 동작)
* OCI/AWS 배포 자동화·Docker 이미지
* 로컬 볼트 ↔ 원격 간 **양방향** 동기화·충돌 처리
* 승인 대기형 쓰기 큐 등 인증·권한 고도화
